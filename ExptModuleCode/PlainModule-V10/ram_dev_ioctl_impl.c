...
long ram_dev_ioctl(struct file *filep,
    ...
    if ((ioctl_id <= RAM_DEV_START_ID) || (ioctl_id>= RAM_DEV_STOP_ID)) {
        pr_err("Sairam_10: %s, IOCTL cmd is invalid: %u\n", __func__, cmd);
        return -EINVAL;
    }
    ...
    if (usr_arg_size != drv_arg_size) {
        pr_err("Sairam_10: %s, IOCTL cmd arg is invalid: %u\n", __func__, cmd);
        return -EINVAL;
    }
	...
	if (copy_from_user(kdata, (void __user *)arg, drv_arg_size) != 0) {
        pr_err("Sairam_10: %s, IOCTL cmd copy encountered error: %u\n", __func__, cmd);
        return -EINVAL;
    }
    ...
    if (ret != 0) {
        pr_err("Sairam_10: %s, IOCTL cmd copy to user space encountered error: %u\n", __func__, cmd);
        return -EFAULT;
    }
...
int ram_dev_ioctl_read_str(struct file *filep,
    ...
    size = strlen(mesg) + 1;
    if (size > arg->rd_size) {
        pr_err("sairam_10: %s, user buffer is too small\n", __func__);
        return -EINVAL;
    }

    pr_err("Sairam_10: %s, IOCTL cmd to read\n", __func__);
    pr_err("Sairam_10: %s, IOCTL cmd returning string size: %u\n", __func__, size);
    pr_err("Sairam_10: %s, IOCTL cmd returning string: %s\n", __func__, mesg);
    arg->rd_size = size;
...
int ram_dev_ioctl_write_str(struct file *filep,
    ...
    if (sizeof(mesg) < arg->wr_size) {
        pr_err("sairam_10: %s, user buffer is too big: %u\n", __func__, arg->wr_size);
        return -EINVAL;
    }

    pr_err("Sairam_10: %s, IOCTL cmd to write\n", __func__);
    pr_err("Sairam_10: %s, IOCTL cmd writing string size: %u\n", __func__, arg->wr_size);
    ...
    pr_err("Sairam_10: %s, IOCTL cmd writing string: %s\n", __func__, &mesg[0]);
...
int ram_dev_ioctl_alloc_sys_memory(struct file *filep,
    ...
    if (buffer == NULL) {
        pr_err("Sairam_10: %s, IOCTL cmd to allocate memory of size failed: %u\n", __func__, alloc_size);
        return -EFAULT;
    }
