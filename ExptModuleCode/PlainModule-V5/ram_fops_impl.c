...
int dev_open(struct inode *inode, struct file *file) {
    pr_err("Sairam_5: Device opened\n");
    return 0;
}

// Release function
int dev_release(struct inode *inode, struct file *file) {
    pr_err("Sairam_5: Device closed\n");
    return 0;
}
...
ssize_t dev_read(struct file *file,
    ...
    if (copy_to_user(buf, device_buffer + *offset, len)) {
        pr_err("Sairam_5: Failed to send data to user space\n");
        return -EFAULT;
    }
    ...
    *offset += len;
    ...
    pr_err("Sairam_5: Read %d bytes from the device\n", bytes_read);
    return bytes_read;
}
...
ssize_t dev_write(struct file *file,
    ...
    if (copy_from_user(device_buffer + *offset, buf, len)) {
        pr_err("Sairam_5: Failed to receive data from user space\n");
        return -EFAULT;
    }
    ...
    *offset += len;
    ...
    pr_err("Sairam_5: Wrote %d bytes to the device\n", bytes_written);
