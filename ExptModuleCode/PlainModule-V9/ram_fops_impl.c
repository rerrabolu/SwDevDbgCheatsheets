...
int ram_open1(struct inode *inode, struct file *file) {
    pr_err("Sairam_9: %s, Device opened\n", __func__);
...
ssize_t ram_read3(struct file *file,
    ...
    if (copy_to_user(buf, device_buffer + *offset, len)) {
        pr_err("Sairam_9: %s Failed to send data to user space\n", __func__);
        return -EFAULT;
    }
    ...
    *offset += len;
    ...
    pr_err("Sairam_9: %s Read %d bytes from the device\n", __func__, bytes_read);
    return bytes_read;
}
...
ssize_t ram_write3(struct file *file,
    ...
    if (copy_from_user(device_buffer + *offset, buf, len)) {
        pr_err("Sairam_9: %s Failed to receive data from user space\n", __func__);
        return -EFAULT;
    }
    ...
    *offset += len;
    ...
    pr_err("Sairam_9: %s Wrote %d bytes to the device\n", __func__, bytes_written);
